...
export default function HeroSection({ name, role, side }) {
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-screen overflow-hidden flex flex-col md:flex-row bg-[#0A1128]">
      ...
      <div className="relative md:w-1/5 w-full py-12 md:py-0 md:h-full z-20 flex flex-col items-center justify-center md:border-l border-[#D4AF37]/20 bg-black/40 backdrop-blur-xl md:backdrop-blur-md">
        
        {/* Tên Cô Dâu Chú Rể */}
        <div className="text-center px-4 mb-8">
          <p className="text-xs md:text-[10px] lg:text-xs tracking-[0.2em] uppercase mb-4 opacity-90 font-semibold text-[#E5D3B3]">
            Tân Lang & Tân Nương
          </p>
          
          <h1 className="text-6xl md:text-5xl lg:text-6xl font-calligraphy text-[#E5D3B3] leading-[1.2] flex flex-col items-center">
            <span>{leftName}</span>
        ...
        <div className="text-center px-6 mt-4">
          <p className="mb-2 text-gray-400 uppercase tracking-[0.2em] text-[10px] md:text-[9px] lg:text-[10px] font-semibold">Save the date</p>
          <div className="w-12 h-[1px] bg-[#D4AF37] mx-auto mb-4" />
          <p className="font-serif text-3xl md:text-2xl lg:text-3xl text-gray-100 mb-2">
            {data.ceremony.dateSolar}
          </p>
          <p className="text-[11px] md:text-[10px] lg:text-[11px] text-gray-400">Nhằm ngày {data.ceremony.dateLunar}</p>
        </div>
      </div>
    </section>
  );
