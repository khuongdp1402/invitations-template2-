...
export default function InvitationWords({ name, role }) {
  ...
  const guestText = name ? `${role ? role + ' ' : ''}${name}` : 'quý khách';

  return (
    <section ref={containerRef} className="py-12 px-4 bg-transparent relative z-10 overflow-hidden">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-center gap-12 md:gap-8">
        ...
        <div className="invitation-item w-full md:w-1/3 text-center px-4">
          ...
          <p className="text-[#4A3728]/80 font-serif leading-relaxed">
            Sự hiện diện của <strong className="font-semibold text-[#4A3728]">{guestText}</strong> là niềm vinh hạnh lớn lao cho gia đình chúng tôi.
          </p>
          
          <div className="w-24 h-px bg-[#D4AF37] mx-auto mt-10" />
        </div>
