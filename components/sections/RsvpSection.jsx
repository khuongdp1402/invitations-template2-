      ...
      <div className="max-w-xl mx-auto bg-white/90 backdrop-blur-md rounded-2xl p-8 shadow-2xl border border-white">
        ...
        {isSubmitted ? (
          <div className="text-center py-12 space-y-4 animate-in fade-in zoom-in duration-500">
            ...
            <h3 className="text-2xl font-serif text-gray-800">Cảm ơn {name || 'bạn'}!</h3>
            <p className="text-gray-600">Chúng tôi đã nhận được phản hồi. Hẹn gặp bạn tại lễ cưới nhé!</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    className="text-amber-500 focus:ring-amber-400"
                  />
                  <span className="text-gray-700">Rất tiếc không thể đến</span>
                </label>
              </div>
            </div>
            ...
            {formData.attendance === 'yes' && (
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                <label className="block text-sm font-medium text-gray-700 mb-1">Số người tham dự</label>
                <select 
                  value={formData.guests}
                  onChange={(e) => setFormData({...formData, guests: e.target.value})}
                  className="w-full px-4 py-2 bg-white border border-gray-200 rounded-md focus:border-amber-400 focus:ring-amber-400 outline-none transition-colors"
                >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Lời chúc gửi tới Cô dâu & Chú rể</label>
              <textarea 
                rows="3" 
                placeholder="Gửi gắm chút tình cảm vào đây nhé..."
                value={formData.wishes}
                onChange={(e) => setFormData({...formData, wishes: e.target.value})}
                className="w-full px-4 py-2 bg-white border border-gray-200 rounded-md focus:border-amber-400 focus:ring-amber-400 outline-none transition-colors resize-none"
              ></textarea>
            </div>
